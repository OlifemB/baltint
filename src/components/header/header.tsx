        ...
        <header className={'bg-[#050624]/80 shadow-xl fixed left-0 top-0 right-0 backdrop-blur-2xl z-100'}>
            <div className={'flex flex-row flex-1 justify-between align-center container mx-auto py-6 px-4 md:px-0'}>
                    ...
                    {componentData.nav.map((item, index) =>
                        <div key={item.title + item.link + index}>
                            <Link
                                replace={true}
                                href={item.link}
                                className={clsx('font-normal hover:text-gray-100  duration-300', item.id === pathname.slice(1) ||pathname === '/' && item.id === 'main' ? 'text-white [text-shadow:_0_0_10px_rgb(255_255_255_/_40%)]' : 'text-gray-300 ')}
                            >
                                {item.title}
                            </Link>
                        </div>
                ...
                <nav
                    id={'mobile-menu'}
                    style={{zIndex: '1000'}}
                    className={clsx('fixed left-0 top-0 left-0 w-full bg-[#1B1C3B] overflow-hidden flex flex-1 w-full flex-col justify-center items-start gap-2 md:hidden duration-300 scale-100', isOpen ? 'h-screen' : 'h-0')}
                >
                    {componentData.nav.map((item, index) =>
                        <div className={'px-8 flex flex-col gap-8'}
                            ...
                            <Link
                                replace
                                href={item.link}
                                className={clsx('font-normal hover:text-gray-100  duration-300 text-3xl', pathname === '/' && item.id === 'main' || item.id === pathname.slice(1) ? 'text-gray-100 font-bold' : 'text-gray-400 ')}
                            >
                                {item.title}
                            </Link>
                        </div>
